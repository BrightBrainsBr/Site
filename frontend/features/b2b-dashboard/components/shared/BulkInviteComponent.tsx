...
export function BulkInviteComponent({
    ...
    reader.onload = (ev) => {
      ...
      for (const line of dataLines) {
        ...
        if (cols[0] && cols[0].includes('@')) {
          emails.push(cols[0])
          if (cols[1] && !csvDept) csvDept = cols[1]
        }
      }
      ...
      if (csvDept && !department) setDepartment(csvDept)
    }
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setResult({ ok: 0, failed: emails.length, errors: ['Erro ao processar convites'] })
        return
      }
      ...
      const errors = results.filter((r) => !r.ok).map((r) => r.error ?? 'Erro desconhecido')
    ...
    <div className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#07111F] p-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {loading ? 'Enviando...' : `Convidar${emailCount > 0 ? ` (${emailCount})` : ''}`}
          </button>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-3 space-y-1">
          {result.ok > 0 && (
            <p className="text-[12px] text-[#34D399]">{result.ok} convite(s) enviado(s) com sucesso.</p>
          )}
          {result.failed > 0 && (
