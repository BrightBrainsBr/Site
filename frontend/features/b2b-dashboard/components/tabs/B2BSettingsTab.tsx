...
interface AccessCodeEntry {
  ...
  used_at: string | null
  used_by_evaluation_id: string | null
  active: boolean
  created_at: string
}
...
type SubTab = 'access' | 'collaborators'
...
export function B2BSettingsTab({ companyId }: B2BSettingsTabProps) {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/b2b/${companyId}/settings`)
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      {/* Sub-tab navigation */}
      <div className="flex gap-1 rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#0E1E33] p-1">
        <button
        ...
        >
          ...
          Colaboradores
          {data?.collaborators?.evaluations?.length ? (
      ...
      {subTab === 'access' && (
        <div className="space-y-6">
          ...
          />
          <InviteSection companyId={companyId!} onInvited={fetchSettings} />
          <UsersSection
            companyId={companyId!}
            users={data?.users ?? []}
            onUpdate={fetchSettings}
      ...
      {subTab === 'collaborators' && (
        <CollaboratorsSection
          ...
          accessCodes={data?.collaborators?.access_codes ?? []}
        />
      )}
    </div>
  )
  ...
  const invite = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMsg({ ok: false, text: data.error ?? 'Erro ao convidar' })
      }
    } catch {
    ...
    <div className="rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#0E1E33] p-5">
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-64 rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#07111F] px-3 py-2 text-[12px] text-[#E2E8F0] placeholder-[#64748B] focus:border-[#14B8A6] focus:outline-none"
        />
        <button
          onClick={invite}
      ...
      {msg && (
        <p className={`mt-2 text-[12px] ${msg.ok ? 'text-[#34D399]' : 'text-[#F87171]'}`}>
          {msg.text}
        </p>
      )}
    </div>
...
function UsersSection({
  ...
  const remove = async (userId: string) => {
    setRemoving(userId)
    try {
      await fetch(`/api/b2b/${companyId}/settings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#0E1E33] p-5">
      ...
      <p className="mb-4 text-[11px] text-[#64748B]">
        Pessoas com acesso ao dashboard desta empresa.
      </p>

      {users.length === 0 ? (
        <p className="text-[12px] text-[#64748B]">Nenhum usuário vinculado.</p>
      ) : (
...
function CollaboratorsSection({
  ...
  const collaborators = useMemo(() => {
    ...
    for (const ac of accessCodes) {
      const key = ac.employee_email ?? ac.id
      if (!map.has(key)) {
        let status = 'code_sent'
        if (ac.used_at) status = 'completed'
        else if (ac.started_at) status = 'started'

        map.set(key, {
          key,
          name: null,
          email: ac.employee_email,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <StatCard label="Completos" value={statusCounts['completed'] ?? statusCounts['report_generated'] ?? 0} color="#34D399" />
        <StatCard label="Em andamento" value={statusCounts['in_progress'] ?? statusCounts['started'] ?? 0} color="#F59E0B" />
...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div
      className="flex-1 rounded-lg border border-[rgba(255,255,255,0.08)] bg-[#0E1E33] px-4 py-3"
