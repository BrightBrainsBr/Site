...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const authListenerRef = useRef<ReturnType<typeof authService.onAuthStateChange>['data'] | null>(null);
    ...
    const { data: authListener } = authService.onAuthStateChange(
      async (event, newSession) => {
        ...
        if (event === 'SIGNED_OUT') {
          ...
          console.log('[AuthProvider] 🔑 User signed in - updating session immediately');
          if (newSession) {
