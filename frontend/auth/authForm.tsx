...
export function AuthForm({ view, redirectedFrom, onSuccess, onError, hideProviders = false, hidePassword = false }: AuthFormProps) {
  ...
  const onSubmit = async (data: AuthFormData) => {
    ...
    try {
      if (view === 'sign_up') {
        ...
        const result = await authService.signUp({ email: data.email, password: data.password });
        ...
        if (result.error) {
          ...
          if (result.error.code === 'too_many_attempts' || 
            ...
            setAuthError('Too many verification attempts. Please wait a minute before trying again, or check your email for a previous verification code.');
  ...
  useEffect(() => {
    const { data: authListener } = supabase.auth.onAuthStateChange((event: string, session: any | null) => {
      console.log(`[AuthForm] Auth Event: ${event}`, session);
      
      if (event === 'SIGNED_IN' || event === 'USER_UPDATED') {
         console.log('[AuthForm] Signed In or User Updated event received.');
         console.log('[AuthForm] Redirect context', { redirectedFrom, hasCompletedOnboarding });
         ...
         if (onSuccess) {
             ...
             const finalRedirect = redirectedFrom || (hasCompletedOnboarding ? '/dashboard' : '/onboarding/basic-info');
          ...
          console.log('[AuthForm] USER_ADDED event - user signed up, redirecting to verify email');
  ...
  }, [supabase, router, hasCompletedOnboarding, redirectedFrom, onSuccess, onError]);
     ...
     <div>
        ...
        {isRedirecting ? (
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-sm text-gray-500">Redirecting to verification...</p>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={submitForm(onSubmit)} className="space-y-4">
                ...
                <>
                  {showGoogleFallback ? (
                    ...
                    >
                      ...
                      {view === 'sign_up' ? 'Sign up with Google' : 'Sign in with Google'}
                    </button>
                  )}
              ...
              {view === 'sign_in' && (
                <div className="text-center">
                  <Link href="/reset-password" className="text-sm text-gray-600 hover:text-gray-700">
