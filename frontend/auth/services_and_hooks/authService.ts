...
export const authService = {
    ...
    try {
      ...
      if (error) {
        if (error.message.toLowerCase().includes('email not confirmed')) {
          console.log('[signIn] User email not confirmed. Initiating OTP resend...');
          
          const resendResult = await this.resendOtp({ email });

          if (resendResult.error) {
            console.error('[signIn] Failed to resend OTP during sign-in attempt:', resendResult.error);
            return { error: resendResult.error };
          }

          console.log('[signIn] OTP resent successfully. Redirecting to verification page.');
  ...
  async signUp({ email, password }: SignupCredentials): Promise<AuthResponse> {
    try {
      ...
      if (signUpData?.user?.identities?.length === 0) {
        console.log('User exists but email not confirmed. Redirecting to verification...');
      ...
      if (signUpData.session && signUpData.user) {
        console.log('Signup successful and email confirmation disabled. Updating store and redirecting to onboarding.');
      ...
      if (!signUpData.session && signUpData.user) {
        console.log('Signup successful, email confirmation enabled. Supabase already sent confirmation email.');
      ...
      return { 
        error: { message: 'Unexpected state after signup.', code: AuthErrorCode.UNKNOWN_ERROR } 
      };
   ...
   */
  async verifyOtp({ email, token }: { email: string; token: string }): Promise<AuthResponse> {
    try {
      ...
      if (data.session && data.user) {
        console.log('OTP verification successful. Updating store and redirecting to onboarding.');
  ...
  async signOut(): Promise<AuthResponse> {
    try {
      ...
      if (!response.ok) {
        console.error('Server-side signout failed:', response.status, await response.text());
   ...
   */
  async resetPassword({ email }: ResetPasswordCredentials): Promise<AuthResponse> {
    try {
      const supabase = createClient();
   ...
   */
  async updatePassword({ password }: UpdatePasswordCredentials): Promise<AuthResponse> {
    try {
      const supabase = createClient();
   ...
   */
  async getUserProfile(userId: string): Promise<AuthResponse<UserProfile | null>> {
    ...
    try {
      ...
      if (error) {
        console.error(`[authService] Supabase error fetching profile for user ${userId}:`, error);
        return {
          data: null,
          error: {
            message: error.message,
      ...
      if (data) {
        console.log(`[authService] Successfully fetched profile for user ${userId}.`);
      } else {
        console.warn(`[authService] No profile found for user ${userId}, returning null.`);
      }
      ...
      const err = error as Error;
      console.error(`[authService] Catch block error fetching profile for user ${userId}:`, err);
      return {
        data: null,
        error: {
  ...
  async forceSignOut(): Promise<AuthResponse> {
    try {
      ...
      try {
        ...
        console.warn("[EMERGENCY] Server endpoint call failed, proceeding with redirect:", apiError);
      }
    ...
    try {
      ...
      if (error) {
        ...
        if (provider === 'google' && error.message.includes('popup')) {
          return {
            error: {
              message: 'Please disable popup blockers and try again. If the issue persists, try using a different browser.',
      ...
      return {
        error: {
          message: err.message || `An error occurred during ${provider} sign in`,
          code: AuthErrorCode.UNKNOWN_ERROR,
        },
      };
    }
...
function mapErrorCodeToAuthErrorCode(errorMessage: string): AuthErrorCode {
  ...
  if (lowerCaseMessage.includes('too many attempts') || 
