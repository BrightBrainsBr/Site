...
export function useSubscriptionLimits() {
  ...
  const { data: limits, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async (): Promise<SubscriptionLimits> => {
      ...
      const maxLimit = SUBSCRIPTION_LIMITS[subscriptionType].maxPersonalizedUseCases;
      ...
      const remainingUseCases = isUnlimited ? -1 : Math.max(0, maxLimit - currentUsage);
  ...
  const checkCanCreate = () => {
    ...
    if (!limits) return { canCreate: false, reason: 'Unable to determine limits' };
  ...
  return {
    ...
    usagePercentage: limits?.maxPersonalizedUseCases === -1 ? 0 : 
      ((limits?.currentPersonalizedUseCases || 0) / (limits?.maxPersonalizedUseCases || 1)) * 100,
  };
}