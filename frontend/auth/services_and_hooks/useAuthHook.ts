...
export function useAuthHook() {
  ...
  const authQuery = useQuery({
    ...
    queryFn: async () => {
      if (isSessionFetchingRef.current) {
        return { session: authState.session, profile: authState.profile, organizations: authState.organizations, subscription: authState.subscription };
      }
      ...
      try {
        ...
        if (sessionRes.error && userRes.error) {
          setSession(null);
          setLoading(false);
          return { session: null, profile: null, organizations: [], subscription: { status: 'free' as const, priceId: null, currentPeriodEnd: null, cancelAtPeriodEnd: null } };
        }
        ...
        if (finalSession?.user?.id) {
          ...
          const subscription = { status: 'free' as const, priceId: null, currentPeriodEnd: null, cancelAtPeriodEnd: null }; // Disabled for Mindless Academy
        ...
        return { session: finalSession, profile: null, organizations: [], subscription: { status: 'free' as const, priceId: null, currentPeriodEnd: null, cancelAtPeriodEnd: null } };
        
      } catch (error) {
        console.error('[useAuthHook] Complete auth fetch error:', error);
        setSession(null);
        setLoading(false);
        return { session: null, profile: null, organizations: [], subscription: { status: 'free', priceId: null, currentPeriodEnd: null, cancelAtPeriodEnd: null } };
      } finally {
  ...
  const primaryOrganization = useMemo(() => {
    return authState.organizations?.find(org => org.is_primary) || authState.organizations?.[0] || null;
