...
export async function diagnoseAuthIssue() {
  ...
  try {
    ...
    console.log('Auth Service Session:', serviceSession.data ? 'Exists' : 'None');
    ...
    if (!!sessionData.session !== !!authStore.session) {
      console.warn('⚠️ MISMATCH: Session existence different between Supabase and Auth Store');
    }
    
    if (!!userData.user !== !!authStore.user) {
      console.warn('⚠️ MISMATCH: User existence different between Supabase and Auth Store');
    }
    
    if (userData.user && authStore.user && userData.user.id !== authStore.user.id) {
      console.warn('⚠️ MISMATCH: User IDs different between Supabase and Auth Store');
    }
    
    // Check localStorage
    const authStoreData = localStorage.getItem('auth-store');
    console.log('Auth Store in localStorage:', authStoreData ? 'Exists' : 'None');
    if (authStoreData) {
      try {
        ...
        if (!!parsedData.state.user !== !!authStore.user) {
          console.warn('⚠️ MISMATCH: User existence different between localStorage and Auth Store memory');
        }
        if (!!parsedData.state.session !== !!authStore.session) {
          console.warn('⚠️ MISMATCH: Session existence different between localStorage and Auth Store memory');
        }
      } catch (e) {
    ...
    const hasMismatch = (!!sessionData.session !== !!authStore.session) || 
