...
export async function GET(
  ...
  const { data: company } = await sb
    .from('companies')
    .select(
      'id, name, allowed_domains, departments, bright_insights_enabled, nr1_process_descriptions, nr1_activities, nr1_preventive_measures, sst_responsible_name, sst_responsible_role, sst_signature_url, cnae, risk_grade, emergency_sop_urls'
    )
    .eq('id', companyId)
    .single()
      ...
      try {
        if (role === 'admin') {
          ...
          if (
            ...
            console.warn(
              `[b2b/settings] invite email failed for ${email}:`,
