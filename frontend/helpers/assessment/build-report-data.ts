...
import { SCALE_RANGES } from '../../components/assessment/constants/scoring-ranges'
...
${data.sintomasAtuais.join(', ')}${data.outrosSintomas ? `\nOutros: ${data.outrosSintomas}` : ''}`)
  ...
  if (data.medicamentos.length > 0) {
    const medLines = data.medicamentos.map(
  ...
  if (data.suplementos.length > 0) {
    const supLines = data.suplementos.map(
  ...
  if (data.fontesEstresse.length > 0)
    lifestyle.push(
      `Fontes de estresse: ${data.fontesEstresse.join(', ')}`
    )
  ...
  if (data.familiaCondicoes.length > 0) {
    s.push(`## Histórico Familiar
Condições: ${data.familiaCondicoes.join(', ')}${data.familiaDetalhes ? `\nDetalhes: ${data.familiaDetalhes}` : ''}`)
  }
  ...
  if (data.laudosAnteriores.length > 0) {
    const laudoLines = data.laudosAnteriores.map(
      (l) =>
